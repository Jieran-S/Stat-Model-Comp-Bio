...
coin_EM <- function(D, k, lambda, theta, tolerance = 1e-2) {
  ...
  while (diff > tolerance) {
    ...
    PofXgivenTheta <- gamma
    for (i in 1:nrow(D)) {
      for (k_i in 1:k) {
          PofXgivenTheta[k_i,i] <- prod(D[i,] * theta[k_i] + (D[i,]-1) * (theta[k_i]-1))*lambda[k_i]
      }
    }
    
    gamma <- apply(PofXgivenTheta, 2, function(x) x/sum(x))
    ...
    ll_obs <- ll_hid
    ll_obs <- sum(log(apply(PofXgivenTheta, 2, sum)))
